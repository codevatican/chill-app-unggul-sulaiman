...
import { setToken, setEmail, setPassword, setEmailInput } from '@/store/redux/authSlice'
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (login) {
        ...
        dispatch(setEmail(login.user.email))
      ...
      <div className='absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 z-10 bg-zinc-900/50 px-8 py-16 rounded-xl max-w-xl w-full'>
        <form className='flex flex-col gap-4'>
          <div className='flex flex-col items-center gap-2 mb-4'>
            ...
            <span className='text-white text-lg font-normal font-myfont'>Selamat Datang Kembali!</span>
          </div>
          ...
          <div className='flex flex-col gap-4'>
            <p className='text-gray-400 font-semibold'>
              Belum Punya Akun?{' '}
              <span onClick={() => navigate('/register')} className='text-white font-bold cursor-pointer ml-2'>
                Daftar
              </span>
            </p>
            <button onClick={handleLogin} className='bg-gray-500 py-3 w-full text-white font-bold rounded-full'>
              Masuk
            </button>
            ...
            <button className='bg-transparent border border-gray-500 py-3 w-full text-white font-bold rounded-full flex gap-2 items-center justify-center'>
