...
import { setSelectedMovieId, setIsModalOpen } from '@/store/redux/movieSlice'
...
const MovieCard = ({ data, index, latestMovieId, isHover, setIsHover, showOverlay }) => {
  ...
  return (
    <>
      {isHover && data.id && data.id === selectedMovieId ? (
        <motion.div
              ...
              <section className='flex flex-col gap-1'>
                ...
                <div className='flex flex-wrap gap-2 text-gray-300 text-sm'>
                  {(movieDetail?.genres || []).map((g, i) => (
                    <span key={g.id}>
                      {g.name}
                      {i !== (movieDetail?.genres?.length || 0) - 1 && ' •'}
                    </span>
                  ))}
                </div>
              </section>
        ...
        <div
          ...
          onMouseEnter={() => {
            if (!data?.id) return
            setIsHover(true)
            dispatch(setSelectedMovieId(data.id))
            getVideoUrl({ movie_id: data.id }).then(setVideoURL)
          }}
        >
          <img
          ...
          {index <= 10 && (
            <div className="absolute top-0 right-4">
              <div className="bg-red-700 px-2 py-3 rounded-bl-md rounded-tr-md text-white text-sm font-medium flex flex-col items-center leading-tight">
                <span>Top</span>
                <span>10</span>
              </div>
            </div>
          ...
          {showOverlay && (
            <div className="absolute bottom-0 left-0 w-full px-2 pb-6 flex justify-between items-end bg-gradient-to-t from-black/70 to-transparent rounded-b-lg">
              <h3 className="text-white text-lg font-bold truncate max-w-[70%]">{data.title}</h3>
